...
class ChatBot extends Component {
  ...
  triggerNextStep = (data) => {
    ...
    if (isEnd) {
      ...
      const trigger = this.getTriggeredStep(
        option?.trigger || currentStep.options[0].trigger,
        currentStep.value
      );
      ...
      currentStep = Object.assign(
        ...
        {
          user: true,
          message: option?.label || currentStep.value,
          trigger,
        }
      );
      ...
      this.setState({ renderedSteps, currentStep, previousStep }, () => {
        if (nextStep.user && !nextStep.options) {
          this.setState({ disabled: false }, () => {
            if (enableMobileAutoFocus || !isMobile()) {
              if (this.input) {
                this.input.focus();
  ...
  submitUserMessage = () => {
    ...
    if (!isInvalid) {
      if (currentStep.options) {
        this.triggerNextStep({ value: inputValue });
        return;
      }
  ...
  renderStep = (step, index) => {
    ...
    const { options, component, user, asMessage } = step;
    const steps = this.generateRenderedStepsById();
      ...
      <div className={`rsc ${className}`}>
        {floating && (
          <>
            ...
            >
              <button onClick={(e) => {}} className="open-chat">
                <MdModeEditOutline size={30} color="#96000f" />
              </button>
...
const devDelay = false;
ChatBot.defaultProps = {
  avatarStyle: {},
  botDelay: devDelay || 1000,
  botName: "The bot",
  bubbleOptionStyle: {},
  bubbleStyle: {},
  cache: false,
